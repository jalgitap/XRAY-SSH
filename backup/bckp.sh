...
date=$(date +"%Y-%m-%d")
myservinfo=$(cat /etc/bckp-bot/servinfo.conf)
my_userid=$(cat /etc/bckp-bot/userid.conf)
pwzip=$(tr -dc Q-Xa-y0-9 </dev/urandom | head -c 13 ; echo '')
name=$(tr -dc Q-xa-y0-9 </dev/urandom | head -c 4 ; echo '')
mkdir -p /root/backup
...
cd /root
zip -rP $pwzip $name-$date.zip backup > /dev/null 2>&1
curl -F chat_id="$my_userid" -F document=@"$name-$date.zip" -F caption="
Server  : $myservinfo
Passwd  : $pwzip
Your IP : $IP" https://api.telegram.org/bot7100277508:AAEb5cCh1-YMiNWQ-o6tLeYci85gcUXEaCA/sendDocument &> /dev/null
cd /root
rm -rf /root/backup
rm -rf /root/$name-$date.zip
